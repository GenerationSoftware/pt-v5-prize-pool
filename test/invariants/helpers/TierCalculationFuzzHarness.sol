        ...
        for (uint8 t = 0; t < numberOfTiers; t++) {
            ...
            for (uint u = 1; u < _userCount+1; u++) {
                ...
                for (uint32 p = 0; p < prizeCount; p++) {
                    uint256 prn = TierCalculationLib.calculatePseudoRandomNumber(userAddress, t, p, winningRandomNumber);
                    if (TierCalculationLib.isWinner(prn, eachUserBalance, _userCount*eachUserBalance, vaultPortion, tierOdds)) {
                        drawPrizeCount++;
                    }
                }
            }
